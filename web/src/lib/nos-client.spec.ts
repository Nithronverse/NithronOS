...
describe('nos-client', () => {
  ...
  describe('Path validation', () => {
    ...
    it('should throw error if path includes /api in development', () => {
      const originalEnv = process.env.NODE_ENV;
      process.env.NODE_ENV = 'development';
      
      expect(() => {
        http.get('/api/v1/ping');
      }).toThrow("Do not include '/api' in paths");
    ...
    it('should allow /setup/ paths', async () => {
      ...
      await http.get('/setup/state');
      
      expect(mockAxiosInstance.get).toHaveBeenCalledWith('/setup/state', { params: undefined });
    });
  ...
  describe('CSRF handling', () => {
    it('should add CSRF header if cookie exists', () => {
      const originalCookie = document.cookie;
      document.cookie = 'nos_csrf=test-token';
      
      // Re-create the client to pick up the cookie
      vi.resetModules();
