...
func (h *SharesHandler) CreateShare(w http.ResponseWriter, r *http.Request) {
	...
	if err := os.MkdirAll(filepath.Dir(req.Path), 0755); err != nil {
		getLogger().Error().Err(err).Str("path", req.Path).Msg("failed to create parent directory")
	...
	if isBtrfs(filepath.Dir(req.Path)) {
		...
		if err := cmd.Run(); err != nil {
			...
			if err := os.MkdirAll(req.Path, os.FileMode(req.Mode)); err != nil {
				getLogger().Error().Err(err).Str("path", req.Path).Msg("failed to create directory")
			...
			if err := os.Chmod(req.Path, os.FileMode(req.Mode)); err != nil {
				getLogger().Error().Err(err).Str("path", req.Path).Msg("failed to set permissions")
	...
	if err := h.ensureGroup(groupName); err != nil {
		getLogger().Warn().Err(err).Str("group", groupName).Msg("failed to create share group")
	}

	// Set ownership to root:root (ACLs will handle actual permissions)
	if err := os.Chown(req.Path, 0, 0); err != nil {
		getLogger().Error().Err(err).Str("path", req.Path).Msg("failed to set ownership")
	...
	if req.RecycleDir != "" {
		...
		if err := os.MkdirAll(recyclePath, 0770); err != nil {
			getLogger().Warn().Err(err).Str("path", recyclePath).Msg("failed to create recycle directory")
		}
	}
...
func (h *SharesHandler) ApplyACLs(w http.ResponseWriter, r *http.Request) {
	...
	if err := cmd.Run(); err != nil {
		getLogger().Warn().Err(err).Str("path", req.Path).Msg("failed to clear ACLs")
	}
	...
	for _, owner := range req.Owners {
		...
		if err := cmd.Run(); err != nil {
			getLogger().Error().Err(err).Str("acl", acl).Msg("failed to apply owner ACL")
		}
		...
		if err := cmd.Run(); err != nil {
			getLogger().Error().Err(err).Str("acl", defaultAcl).Msg("failed to apply default owner ACL")
		}
	}
	...
	for _, reader := range req.Readers {
		...
		cmd := exec.Command("setfacl", "-m", acl, req.Path)
		if err := cmd.Run(); err != nil {
			getLogger().Error().Err(err).Str("acl", acl).Msg("failed to apply reader ACL")
		}
		...
		cmd = exec.Command("setfacl", "-m", defaultAcl, req.Path)
		if err := cmd.Run(); err != nil {
			getLogger().Error().Err(err).Str("acl", defaultAcl).Msg("failed to apply default reader ACL")
		}
	}
...
func (h *SharesHandler) WriteSambaConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err := os.MkdirAll(confDir, 0755); err != nil {
		getLogger().Error().Err(err).Str("dir", confDir).Msg("failed to create Samba config directory")
	...
	if err := os.WriteFile(tmpPath, []byte(req.Config), 0644); err != nil {
		getLogger().Error().Err(err).Str("path", tmpPath).Msg("failed to write Samba config")
	...
	if err := cmd.Run(); err != nil {
		os.Remove(tmpPath)
		getLogger().Error().Err(err).Msg("Samba config validation failed")
	...
	if err := os.Rename(tmpPath, confPath); err != nil {
		os.Remove(tmpPath)
		getLogger().Error().Err(err).Str("path", confPath).Msg("failed to install Samba config")
...
func (h *SharesHandler) RemoveSambaConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err := os.Remove(confPath); err != nil && !os.IsNotExist(err) {
		getLogger().Error().Err(err).Str("path", confPath).Msg("failed to remove Samba config")
...
func (h *SharesHandler) ReloadSamba(w http.ResponseWriter, r *http.Request) {
	...
	if err := cmd.Run(); err != nil {
		getLogger().Error().Err(err).Msg("failed to reload Samba")
...
func (h *SharesHandler) WriteNFSExport(w http.ResponseWriter, r *http.Request) {
	...
	if err := os.MkdirAll(exportDir, 0755); err != nil {
		getLogger().Error().Err(err).Str("dir", exportDir).Msg("failed to create exports directory")
	...
	if err := os.WriteFile(exportPath, []byte(req.Config), 0644); err != nil {
		getLogger().Error().Err(err).Str("path", exportPath).Msg("failed to write NFS export")
...
func (h *SharesHandler) RemoveNFSExport(w http.ResponseWriter, r *http.Request) {
	...
	if err := os.Remove(exportPath); err != nil && !os.IsNotExist(err) {
		getLogger().Error().Err(err).Str("path", exportPath).Msg("failed to remove NFS export")
...
func (h *SharesHandler) ReloadNFS(w http.ResponseWriter, r *http.Request) {
	...
	if err := cmd.Run(); err != nil {
		getLogger().Error().Err(err).Msg("failed to reload NFS exports")
...
func (h *SharesHandler) CreateSubvol(w http.ResponseWriter, r *http.Request) {
	...
	cmd := exec.Command("btrfs", "subvolume", "create", req.Path)
	if err := cmd.Run(); err != nil {
		getLogger().Error().Err(err).Str("path", req.Path).Msg("failed to create subvolume")
...
func (h *SharesHandler) EnsureGroup(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.ensureGroup(req.Name); err != nil {
		getLogger().Error().Err(err).Str("group", req.Name).Msg("failed to ensure group")
...
func (h *SharesHandler) ReloadAvahi(w http.ResponseWriter, r *http.Request) {
	...
	if err := cmd.Run(); err != nil {
		getLogger().Error().Err(err).Msg("failed to reload Avahi")
