...
func InitializeSharesConfig(logger *zerolog.Logger) error {
	...
	logger.Info().Msg("Creating initial shares configuration")
	...
	if err := setSharesConfigOwnership(); err != nil {
		logger.Warn().Err(err).Msg("failed to set shares config ownership")
	}

	logger.Info().Str("path", SharesConfigPath).Msg("Created initial shares configuration")
	return nil
}
...
func MigrateExistingShares(logger *zerolog.Logger) error {
	// Check Samba configs
	sambaDir := "/etc/samba/smb.conf.d"
	if err := markExistingConfigs(sambaDir, ".conf"); err != nil {
		logger.Warn().Err(err).Msg("failed to migrate Samba configs")
	}

	// Check NFS exports
	nfsDir := "/etc/exports.d"
	if err := markExistingConfigs(nfsDir, ".exports"); err != nil {
		logger.Warn().Err(err).Msg("failed to migrate NFS exports")
	}
...
func markExistingConfigs(dir, suffix string) error {
	...
	for _, entry := range entries {
		...
		if err := addManagedHeader(path); err != nil {
			logger.Warn().Str("file", path).Err(err).Msg("failed to add managed header")
		}
	}
...
func addManagedHeader(path string) error {
	...
	logger.Info().Str("file", path).Msg("Marked existing config as preserved")
	return nil
}
...
func RunMigration() error {
	logger := zerolog.New(os.Stderr).With().Timestamp().Logger()
	logger.Info().Msg("Running shares migration")

	// Initialize shares.json if needed
	if err := InitializeSharesConfig(&logger); err != nil {
		return fmt.Errorf("failed to initialize shares config: %w", err)
	}

	// Mark existing configs
	if err := MigrateExistingShares(&logger); err != nil {
		return fmt.Errorf("failed to migrate existing shares: %w", err)
	}

	logger.Info().Msg("Shares migration completed")
	return nil
}
